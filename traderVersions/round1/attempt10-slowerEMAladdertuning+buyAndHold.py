...
class Trader:
    ...
    def trade_osmium(self, product: str, order_depth: OrderDepth, position: int, trader_data: dict):
        ...
        # === STEP 4: THE SELF-TUNING STICKY LADDER ===
        scales = [0.25, 0.50, 0.75, 1.00, 1.25]
        distances = [max(1, int(round(spread_ema * scale))) for scale in scales]
        distances = sorted(list(set(distances))) 

        buy_levels = [fair_value - d for d in distances]
        sell_levels = [fair_value + d for d in distances]
        
        vol_weights = [0.35, 0.25, 0.20, 0.15, 0.05]
        ...
        if can_buy > 0:
            for i in range(len(buy_levels)):
                ...
                if i == len(buy_levels) - 1:
                    ...
                    vol = int(base_can_buy * vol_weights[i])

                if vol > 0:
                    orders.append(Order(product, int(level), int(vol)))
                    can_buy -= vol
        ...
        if can_sell > 0:
            for i in range(len(sell_levels)):
                ...
                if i == len(sell_levels) - 1:
                    ...
                    vol = int(base_can_sell * vol_weights[i])
